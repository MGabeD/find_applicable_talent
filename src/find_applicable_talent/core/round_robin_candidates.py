...
class Role(BaseModel):
    ...
    available_candidates: Dict[str, str] = {}
    failed_ids: List[str] = []

    def __init__(self, available_candidates: List[str]):
...
class RecruitmentReasoner:
    def __init__(self, candidates: List[Candidate]):
        self.candidates = {c.id: c.deep_copy() for c in candidates}
        self.roles = [Role(list(self.candidates.keys())) for _ in range(5)]
    ...
    def round_robin_single_role(self, role: Role):
        ...
        def process_batch(batch_ids: List[str]):
            ...
            # If we get here, batch failed
            logger.warning(f"Batch failed after {MAX_RETRIES} attempts.")
            for cid in batch_ids:
                role.fail_candidate(cid)

        with ThreadPoolExecutor(max_workers=MAX_WORKERS_PER_ROLE) as executor:
            futures = [executor.submit(process_batch, batch) for batch in candidate_batches]
            for future in as_completed(futures):
                future.result()  


    def assign_all_via_round_robin(self):
        with ThreadPoolExecutor(max_workers=MAX_ROLE_WORKERS) as executor:
            futures = [
                executor.submit(self.round_robin_single_role, role)
                for role in self.roles
            ]
            for future in as_completed(futures):
                try:
    ...
    def update_roles_with_criteria(self, input_roles:List[Dict[str, str]]) -> None:
        ...
        for i in range(len(input_roles)):
            if input_roles[i]["title"] and input_roles[i]["justification"]:
                logger.info(f"Generating rubric for role: {input_roles[i]['title']}")
                rubric = run_prompt_with_parser(
                    build_candidate_criteria_prompt,
                    return_value_from_response,
                    self.model,
                    input_roles[i]["title"],
                    input_roles[i]["justification"]
                )
                self.roles[i].rubric = rubric
                self.roles[i].title = input_roles[i]["title"]
                self.roles[i].justification = input_roles[i]["justification"]
            else:
                logger.warning(f"Skipping rubric generation for incomplete role: {input_roles[i]}")


def define_initial_roles(user_context: str) -> List[Dict[str, str]]:
    model = get_model()
    return run_prompt_with_parser(
...
def refine_roles(current_roles: List[Dict[str, str]], user_feedback: str) -> List[Dict[str, str]]:
    model = get_model()
    return run_prompt_with_parser(
