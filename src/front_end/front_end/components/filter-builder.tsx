...
export function FilterBuilder() {
  ...
  const [currentValueType, setCurrentValueType] = useState<"text" | "number" | "boolean" | "date">("text")

  const addFilter = () => {
    if (currentFilter.path && (currentFilter.value || currentValueType === "boolean")) {
      // For boolean values, use "true" or "false" as the value
      const value = currentValueType === "boolean" ? currentFilter.value || "false" : currentFilter.value
  ...
  const applyFilters = () => {
    // This would send the filters to your backend
    console.log("Sending filters to backend:", filters)
    alert(`Filters sent to backend: ${JSON.stringify(filters, null, 2)}`)
  }
  ...
  const pathOptions = [
    ...
    // Salary
    { value: "annual_salary_expectation.amount", label: "Salary Amount", type: "number" },
    { value: "annual_salary_expectation.currency", label: "Salary Currency", type: "text" },
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <div className="grid grid-cols-3 gap-2">
            <div>
              <Select value={currentFilter.path} onValueChange={handlePathChange}>
                <SelectTrigger>
            ...
            <div>
              <Select
                ...
                onValueChange={(value) => setCurrentFilter({ ...currentFilter, operator: value as Operator })}
              >
                <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {filters.map((filter, index) => {
                const pathLabel = pathOptions.find((p) => p.value === filter.path)?.label || filter.path
      ...
      <CardFooter>
        <Button className="w-full" onClick={applyFilters} disabled={filters.length === 0}>
          Apply Filters
        </Button>
      </CardFooter>
    </Card>
